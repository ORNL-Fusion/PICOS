...
RUN_LABEL=${RUN_LABEL:-p262144_100us_coll_mpexprof_nersc}
...
for tag in "${CASES[@]}"; do
  ...
  if [ ! -f "${input_file}" ] || [ ! -f "${ion_file}" ]; then
    ...
    echo "  python scripts/run_mpex_triplet_picos_fortran.py \\" >&2
    echo "    --setup-only --skip-smooth --picos-run-mode nonrel \\" >&2
